...
function build_safepkt_backend_image() {
    ...
    if [ -z "${RVT_DIRECTORY}" ] || [ ! -d "${RVT_DIRECTORY}" ]; then
...
function pull_rvt_image() {
    source ./.env
    if [ "${RVT_DOCKER_IMAGE}" ] && [ -d "${RVT_DOCKER_IMAGE}" ]; then
...
function copy_configuration_file() {
    ...
    pattern_list_uploaded_sources='s#UPLOADED_SOURCES_LISTING_SCRIPT="list-uploaded-sources"#VERIFICATION_SCRIPT="'"${list_uploaded_sources_script_path}"'"#g'
